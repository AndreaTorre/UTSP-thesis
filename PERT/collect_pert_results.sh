...
if [[ ! -d "$PERT_ROOT" ]]; then
    echo "Errore: directory non trovata: $PERT_ROOT" >&2
    exit 1
fi
...
batch_dirs=("$PERT_ROOT"/RISULTATI_*/batch_sweep/batch_*/ "$PERT_ROOT"/RISULTATI_*/batch_sweep/BATCH_*/)
shopt -u nullglob nocaseglob
batch_dirs=($(printf '%s\n' "${batch_dirs[@]}" | sort -u))

if [[ ${#batch_dirs[@]} -eq 0 ]]; then
    echo "Errore: nessuna cartella RISULTATI_*/batch_sweep/BATCH_*/ trovata in $PERT_ROOT" >&2
    exit 1
fi
