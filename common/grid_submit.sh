...
for B in $BATS; do
  ...
  JOBID=$(sbatch --parsable \
    ...
    --export=ALL,TESI_ROOT_DIR="$UTSP_ROOT" <<EOF_JOB
#!/usr/bin/env bash
source "$UTSP_ROOT/common/env.sh"
python grid_search.py --run-chunk \\\$SLURM_ARRAY_TASK_ID \\
  --exp $EXP --nodes $N --batch $B --chunk-size $CHUNK_SIZE
EOF_JOB
)
  ...
  sbatch --parsable \
    ...
    --export=ALL,TESI_ROOT_DIR="$UTSP_ROOT" <<EOF_JOB > /dev/null
#!/usr/bin/env bash
source "$UTSP_ROOT/common/env.sh"
python grid_search.py --collect --exp $EXP --nodes $N --batch $B
EOF_JOB
