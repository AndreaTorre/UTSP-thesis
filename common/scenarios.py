...
def find_frequent_arcs(
    ...
    if checkpoint_path         is None: checkpoint_path          = os.path.join(OUTPUT_DIR, "pkl", "calib_partial.pkl")

    I_set = {canon_edge(i, j) for (i, j) in I}

    # NOTA: la chiave identifica la calibrazione. Se seme/I/n_scenari cambiano,
    # il checkpoint vecchio non è valido e si riparte da zero (non da dati sporchi).
    state_key = (calibration_seed, n_calibration_scenarios, tuple(sorted(I_set)))
    ...
    if os.path.exists(checkpoint_path):
        ...
        if state.get("key") == state_key:
            ...
            print(f"  Ripreso checkpoint: {len(done_ids)}/{n_calibration_scenarios} scenari già risolti")
        else:
            print("  Checkpoint trovato ma con parametri diversi (seme/I/n_scenari): riparto da zero")
    ...
    for sid in range(1, n_calibration_scenarios + 1):
        ...
        pi_res = solve_exact_tsp(
            ...
            time_limit=CALIB_TIME_LIMIT, mip_gap=CALIB_MIP_GAP,
        )
        ...
        # Salvataggio atomico (tmp + rename): un crash a metà scrittura
        # non corrompe il checkpoint precedente.
...
def generate_scenarios(
    ...
    for scenario_id in scenario_ids:
        if coords is not None and wind is not None:
...
# ---------------------------------------------------------------------------
# DIAGNOSTICA SCENARI (analisi variabilità perturbazioni ERA5)
# ---------------------------------------------------------------------------

def scenario_dist_tensor(results, nodes):
    """
    Tensore (n_scenari, n, n) delle distanze perturbate per scenario.
    Input: results da generate_scenarios/generate_scenario_batches.
    Ordine scenari = sorted(results.keys()); ordine nodi = ordine di `nodes`.
    Funzione pura: nessun I/O, il salvataggio è a carico del chiamante.
    """
    scenario_ids = sorted(results.keys())
    if len(scenario_ids) < 2:
        raise ValueError("Servono almeno 2 scenari per un'analisi di variabilità.")

    T = np.array([
        [[results[sid]["scenario_dist"][i][j] for j in nodes] for i in nodes]
        for sid in scenario_ids
    ])
    return T, scenario_ids
