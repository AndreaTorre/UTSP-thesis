...
def plot_utsp_graph_weights(exp_name, nodes, coords, H, threshold=0.01, title_suffix="", I=None, save=True):
    import numpy as np
    ...
    for i in nodes:
        for j in nodes:
            ...
            ax.annotate("", xy=(xj, yj), xytext=(xi, yi),
                        arrowprops=dict(arrowstyle="->", color=color, lw=lw, mutation_scale=ms), zorder=2)
...
def plot_utsp_random_scenario_graphs(exp_name, nodes, coords, scenario_ids, H_list, I, n_samples=5, seed=42):
    scenario_ids = list(scenario_ids)
    if len(scenario_ids) != len(H_list):
        raise ValueError("scenario_ids e H_list devono avere la stessa lunghezza.")

    rng = random.Random(seed)
    selected = rng.sample(range(len(scenario_ids)), min(n_samples, len(scenario_ids)))

    for k in selected:
        sid = scenario_ids[k]
        H_s = H_list[k].detach().squeeze(0).cpu().numpy()
        plot_utsp_graph_weights(f"{exp_name}_scenario_{sid}", nodes, coords, H_s,
                                title_suffix=f"(scenario {sid})", I=I)

    return [scenario_ids[k] for k in selected]
