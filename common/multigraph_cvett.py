...
# ─────────────────────────────────────────────────────────────────────
def _build_graph_instance(nodes, coords, base_dist, E, K, seed, wind=None):
    ...
    # CVETT: coords+wind ⇒ perturbazione da VENTO (stesso campo per tutte le geometrie).
...
def _load_cvett_wind():
    """Vento Montrealese (WIND_NC_PATH_TRAIN) condiviso da tutte le geometrie."""
    from wind_perturbation import load_wind_field
    import config
    w = load_wind_field(config.WIND_NC_PATH_TRAIN)
    print(f"[multigraph_cvett] vento caricato da {config.WIND_NC_PATH_TRAIN}")
    return w
...
def generate_graph_instances(n_instances, K, mode, seed=UTSP_TRAINING_SEED,
    ...
    for g in range(n_instances):
        ...
        inst = _build_graph_instance(nodes, coords, base_dist, E, K, seed=g_seed, wind=_wind)
        if len(inst["nodes"]) != N:
